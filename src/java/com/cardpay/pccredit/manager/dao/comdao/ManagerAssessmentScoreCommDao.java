...
public class ManagerAssessmentScoreCommDao {
	...
	public Double sumAppApplyQuota(List<String> customerIds) {
		...
		sb.append(")");
		String sql = "select ifnull(sum(t.actual_quote), 0) as ACTUALQUOTE from customer_application_info t " +
		...
		Double b =  (Double) list.get(0).get("ACTUALQUOTE");
		return b;
	}
	...
	public List<HashMap<String, Object>> sumAccountBill(List<String> customerIds, int year, int month) {
		...
		sb = sb.deleteCharAt(sb.length() - 1);
		sb.append(")");
		String sql = "select ifnull(sumt.OVERDRAFT_ACCOUNT), 0) as OVERDRAFT_ACCOUNT," +
			"ifnull(sumt.PAID_INTEREST_ACCOUNT), 0) as PAID_INTEREST_ACCOUNT," +
			"ifnull(sum(t.CURRENT_MONTH_INTEREST_ACCOUNT), 0) as CURRENT_MONTH_INTEREST_ACCOUNT," +
			"ifnull(sum(t.OVERDUE_AMOUNT), 0) as OVERDUE_AMOUNT," +
			"ifnull(sum(t.PAYBACK_ACCOUNT), 0) as PAYBACK_ACCOUNT," +
			"ifnull(sum(t.BALANCE_ACCOUNT), 0) as BALANCE_ACCOUNT" +
	...
	public Double perDayAverageCreditlineAccount(List<String> managerIds, int year, int month) {
		...
		sb = sb.deleteCharAt(sb.length() - 1);
		sb.append(")");
		String sql = "select ifnull(sum(t.AVERAGE_DAILY_OVERDRAFT), 0) as AVERAGE_DAILY_OVERDRAFT" +
	...
	public Double sumOverduePaybackAll(List<String> customerIds) {
		...
		sb = sb.deleteCharAt(sb.length() - 1);
		sb.append(")");
		String sql = "select ifnull(sum(t.OVERDUE_PAYBACK_ALL), 0) as OVERDUE_PAYBACK_ALL from customer_overdue t " +
