package com.cardpay.pccredit.customer.model;

import com.wicresoft.jrad.base.database.id.IDType;
import com.wicresoft.jrad.base.database.model.BusinessModel;
import com.wicresoft.jrad.base.database.model.ModelParam;

/**
 * 
 * @author 贺珈
 * 原始客户信息--家庭成员（太原）
 *
 */
@ModelParam(table = "ty_customer_rygl",generator=IDType.assigned)
public class CustomerFirsthendRygl extends BusinessModel{
	private static final long serialVersionUID = 1L;
	private String dm;//
	private String xm;//
	private String py;//
	private String xb;//
	private String ssjg;//
	private String zw;//
	private String bgdh;//
	private String sjhm;//
	private String czdh;//
	private String dzxx;//
	private String yzbm;//
	private String dz;//
	private String bz;//
	private String userid;//
	private String ssbm;//
	private String gh;//
	private String iskhjl;//
	private String ywjs;//
	private String dldm;//
	private String ddrq;//
	private String whry;//
	private String dcjg;//
	private String khjldj;//
	private String sfzhm;//
	private String tsrq;//
	private String sgzh;//
	private String pxzh;//
	private String createTime;
	public String getDm() {
		return dm;
	}
	public void setDm(String dm) {
		this.dm = dm;
	}
	public String getXm() {
		return xm;
	}
	public void setXm(String xm) {
		this.xm = xm;
	}
	public String getPy() {
		return py;
	}
	public void setPy(String py) {
		this.py = py;
	}
	public String getXb() {
		return xb;
	}
	public void setXb(String xb) {
		this.xb = xb;
	}
	public String getSsjg() {
		return ssjg;
	}
	public void setSsjg(String ssjg) {
		this.ssjg = ssjg;
	}
	public String getZw() {
		return zw;
	}
	public void setZw(String zw) {
		this.zw = zw;
	}
	public String getBgdh() {
		return bgdh;
	}
	public void setBgdh(String bgdh) {
		this.bgdh = bgdh;
	}
	public String getSjhm() {
		return sjhm;
	}
	public void setSjhm(String sjhm) {
		this.sjhm = sjhm;
	}
	public String getCzdh() {
		return czdh;
	}
	public void setCzdh(String czdh) {
		this.czdh = czdh;
	}
	public String getDzxx() {
		return dzxx;
	}
	public void setDzxx(String dzxx) {
		this.dzxx = dzxx;
	}
	public String getYzbm() {
		return yzbm;
	}
	public void setYzbm(String yzbm) {
		this.yzbm = yzbm;
	}
	public String getDz() {
		return dz;
	}
	public void setDz(String dz) {
		this.dz = dz;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	public String getUserid() {
		return userid;
	}
	public void setUserid(String userid) {
		this.userid = userid;
	}
	public String getSsbm() {
		return ssbm;
	}
	public void setSsbm(String ssbm) {
		this.ssbm = ssbm;
	}
	public String getGh() {
		return gh;
	}
	public void setGh(String gh) {
		this.gh = gh;
	}
	public String getIskhjl() {
		return iskhjl;
	}
	public void setIskhjl(String iskhjl) {
		this.iskhjl = iskhjl;
	}
	public String getYwjs() {
		return ywjs;
	}
	public void setYwjs(String ywjs) {
		this.ywjs = ywjs;
	}
	public String getDldm() {
		return dldm;
	}
	public void setDldm(String dldm) {
		this.dldm = dldm;
	}
	public String getDdrq() {
		return ddrq;
	}
	public void setDdrq(String ddrq) {
		this.ddrq = ddrq;
	}
	public String getWhry() {
		return whry;
	}
	public void setWhry(String whry) {
		this.whry = whry;
	}
	public String getDcjg() {
		return dcjg;
	}
	public void setDcjg(String dcjg) {
		this.dcjg = dcjg;
	}
	public String getKhjldj() {
		return khjldj;
	}
	public void setKhjldj(String khjldj) {
		this.khjldj = khjldj;
	}
	public String getSfzhm() {
		return sfzhm;
	}
	public void setSfzhm(String sfzhm) {
		this.sfzhm = sfzhm;
	}
	public String getTsrq() {
		return tsrq;
	}
	public void setTsrq(String tsrq) {
		this.tsrq = tsrq;
	}
	public String getSgzh() {
		return sgzh;
	}
	public void setSgzh(String sgzh) {
		this.sgzh = sgzh;
	}
	public String getPxzh() {
		return pxzh;
	}
	public void setPxzh(String pxzh) {
		this.pxzh = pxzh;
	}
	public String getCreateTime() {
		return createTime;
	}
	public void setCreateTime(String createTime) {
		this.createTime = createTime;
	}
	

}
